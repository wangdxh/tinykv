...
func (d *storeWorker) maybeCreatePeer(regionID uint64, msg *rspb.RaftMessage) (bool, error) {
	...
	if !util.IsInitialMsg(msg.Message) {
		mylog.Basic("target peer %s doesn't exist", msg.ToPeer)
		return false, nil
	}
	...
	for _, region := range meta.getOverlapRegions(&metapb.Region{
		...
		mylog.Basic("peer %d  to peer %d  msg %s is overlapped with exist storeid %d region %s",
			msg.FromPeer.Id, msg.ToPeer.Id, msg, d.id, region)
